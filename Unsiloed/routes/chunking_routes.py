...
@router.post("/chunking/streaming")
async def chunk_document_streaming(
    ...
    """
    Chunk a document file (PDF, DOCX, PPTX) using streaming extraction for better performance
    ...
    Returns:
        JSON response with chunks and metadata
    """
    logger.info(f"Received request for streaming document chunking using {strategy} strategy")
    logger.debug(f"Document file name: {document_file.filename}")
    ...
    try:
        ...
        # Process the document with the requested chunking strategy using streaming extraction
        result = process_document_chunking_streaming(
            file_path, file_type, strategy, chunk_size, overlap
        )

        logger.info(f"Streaming document chunking completed with {result['total_chunks']} chunks")
        return JSONResponse(content=result)

    except Exception as e:
        logger.error(f"Error processing document with streaming: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Error processing document: {str(e)}"
        )
...
@router.post("/chunking/enhanced")
async def chunk_document_enhanced(
    document_file: UploadFile,
    strategy: ChunkingStrategy = Form("semantic"),
    chunk_size: int = Form(1000),
    overlap: int = Form(100),
    ...
    """
    Chunk a document file (PDF, DOCX, PPTX) using enhanced streaming for better performance.
    Uses improved extraction techniques for better results with complex documents.
    ...
    Returns:
        JSON response with chunks and metadata
    """
    logger.info(f"Received request for enhanced streaming chunking using {strategy} strategy")
    logger.debug(f"Document file name: {document_file.filename}")
    ...
    try:
        ...
        # Process the document with the streaming chunking pipeline
        result = process_document_chunking_streaming(
            file_path, file_type, strategy, chunk_size, overlap
        )

        logger.info(f"Enhanced streaming chunking completed with {result['total_chunks']} chunks")
        return JSONResponse(content=result)

    except Exception as e:
        logger.error(f"Error processing document with enhanced streaming: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Error processing document: {str(e)}"
        )
