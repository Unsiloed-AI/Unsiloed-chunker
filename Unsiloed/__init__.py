...
async def process_streaming(options):
    """
    Process a document file with OCR and chunking capabilities using streaming extraction
    ...
    try:
        ...
        # Process the document with streaming extraction
...
def process_streaming_sync(options):
    """Synchronous version of the process_streaming function"""
    # Get file path
    file_path = options.get("filePath")
    if not file_path:
        raise ValueError("filePath is required")
...
def process_streaming_sync(options):
    ...
    result = loop.run_until_complete(process_streaming(options))
    loop.close()
    return result
