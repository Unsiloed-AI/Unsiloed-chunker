...
import Unsiloed
from test_config import PDF_TEST_FILE, TEST_RESULTS_DIR
...
def test_standard_processing():
    print("Testing standard processing for PDF...")
    times = []
    total_chunks = 0
    result = None
    ...
    # Save results to file for PDF standard processing
    with open(os.path.join(TEST_RESULTS_DIR, 'pdf_standard_time.txt'), 'w') as f:
        f.write(f"{avg_time}\n{total_chunks}")
    
    with open(os.path.join(TEST_RESULTS_DIR, 'pdf_standard_result.json'), 'w') as f:
        json.dump(result, f, indent=2)
        
    return avg_time, total_chunks

def test_streaming_processing():
    print("\nTesting streaming processing for PDF...")
    times = []
    total_chunks = 0
    result = None
    ...
    # Save results to file for PDF streaming processing
    with open(os.path.join(TEST_RESULTS_DIR, 'pdf_streaming_time.txt'), 'w') as f:
        f.write(f"{avg_time}\n{total_chunks}")
        
    with open(os.path.join(TEST_RESULTS_DIR, 'pdf_streaming_result.json'), 'w') as f:
        json.dump(result, f, indent=2)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PDF_TEST_FILE):
        print(f"Error: Test document {PDF_TEST_FILE} not found.")
        print("Please ensure the PDF file exists in the test_data directory.")
        exit(1)

    print(f"Running PDF performance tests on {PDF_TEST_FILE}")
